...
;
; 08H = set or reset selected bit, maybe inverted
; 01H = select bus override
; 02H = select movbot
; 04H = select boot/diag
; 05H = select int enable
...
;
;                               ; XXX SMBAKER USE 80/24 PORTS
SICP0   EQU     0D8H            ; INITIALIZATION COMMAND PORT 0 (ICW1)
SICP1   EQU     0D9H            ; INITIALIZATION COMMAND PORT 1 (ICW2)
SOCP0   EQU     0D8H            ; OPERATION COMMAND PORT 0 (EOI)
SOCP1   EQU     0D9H            ; OPERATION COMMAND PORT 1 (MASK)
...
; TTY I/O PORTS
;
;                               ; XXX SMBAKER CHANGE PORTS FOR 80/24 SERIAL
TTYI    EQU     0ECH            ; TTY INPUT DATA PORT
TTYO    EQU     0ECH            ; TTY OUTPUT DATA PORT
TTYS    EQU     0EDH            ; TTY INPUT STATUS PORT
TTYC    EQU     0EDH            ; TTY OUTPUT CONTROL PORT
...
;
;                               ; XXX SMAKER UPDATE PORTS FOR 80/24
CTR0P   EQU     0DCH            ; LOAD COUNTER 0 OUTPUT COMMAND PORT
CTR1P   EQU     0DCH            ; LOAD COUNTER 1 OUTPUT COMMAND PORT
CTR2P   EQU     0DEH            ; LOAD COUNTER 2 OUTPUT COMMAND PORT
ITCP    EQU     0DFH            ; INTERVAL TIMER OUTPUT COMMAND PORT
...
;        OUT     CPUC
        ...
        MVI     A,01H           ; XXX SMBAKER
        OUT     DEBUG           ; XXX SMBAKER
        ...
        MVI     A,lo(~INT0)     ; INITIALIZE MASK REGISTER
        OUT     SOCP1           ;       FOR SYSTEM 8259
        MVI     A,lo(~INTA)     ; INITIALIZE MASK REGISTER
...
;
        LXI     H,0F7FFH        ; XXX SMBAKER SKIP RAM CHECK AND ASSUME FULL RAM
        JMP     BS2

        IF      0               ; XXX SMBAKER DISABLE RAM CHECK
...
BS1X:
        ...
        ENDIF                   ; XXX SMBAKER - END DISABLE RAM CHECK
...
;
        MVI     A,010H          ; XXX SMBAKER
        OUT     DEBUG           ; XXX SMBAKER
;        CALL    DIAGBT         XXX SMBAKER
;
...
ILOC:   DB      lo(~INT0)       ; INTERRUPT MASK
...
CSLOC:
        JMP     0
ENDX:                           ; THIS LABEL SHOULD BE AT 0EB00H.
;*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*
...
;   JUMP TO 'RI' ROUTINE. IF IT IS USER-DEFINED DEVICE, JUMP TO AUSER.          ;
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
CI:                             ; LOCAL CONSOLE INPUT
        LDA     IOBYT           ; GET STATUS BYTE
        ANI     lo(~CMSK)       ; LOOK AT ONLY CONSOLE FIELD
...
RI8:
        ...
        JZ      AUSER           ; READER USER-DEFINED DEVICE 1
        MVI     A,R2LOC AND HMSK
;*******JMP     AUSER           ; READER USER-DEFINED DEVICE 2
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
; 'AUSER' - ENTERED VIA JUMPS FROM 'LO','LOM','RI','CI','BLK','COM',
...
COM:                            ; LOCAL CONSOLE OUTPUT
        LDA     IOBYT           ; GET STATUS BYTE
        ANI     lo(~CMSK)       ; LOOK ONLY AT CONSOLE FIELD
...
CO0:
        ...
        JNZ     AUSER           ; JUMP IF IT ISN'T, I.E. CONSOLE IS
...
PO1:
        ...
        JZ      AUSER           ; PUNCH = USER DEFINED PUNCH 1
        MVI     A,P2LOC AND 0FFH
        JMP     AUSER           ; PUNCH = USER DEFINED PUNCH 2
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
...
LOM:                            ; LIST OUTPUT ON CONSOLE
        LDA     IOBYT
        ANI     lo(~CMSK)       ; LOOK ONLY AT CONSOLE FIELD OF IOBYT
...
LO:                             ; LIST OUTPUT
        LDA     IOBYT           ; GET STATUS BYTE
        ANI     lo(~LMSK)       ; LOOK AT LIST FIELD
        ...
        JZ      AUSER           ; JUMP IF LIST = USER-DEFINED DEVICE
;-------------------------------
; LIST = LPT
        PUSH    B               ; SAVE BC
LP0:
...
RESTART:
        ...
        MVI     A,lo(~INT0)     ; SET MONITOR'S DEFAULT INTERRUPT MASK
